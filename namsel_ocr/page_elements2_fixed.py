...
#                 cv.rectangle(self.img_arr, (x,y), (x+w, y+h), 0)
            ...
            # Check if this could be a valid Tibetan punctuation before filtering by size
                ...
                if quick_prd == '་':
                    # Adaptive tsheg detection based on measured line characteristics
                    area = cbox[2] * cbox[3]
                    
                    # Use statistical measurements if available
                    if hasattr(self, 'tsek_mean') and self.tsek_mean > 0:
                        # Fine-tuned adaptive thresholds - more inclusive to catch legitimate tshegs
                        min_width = max(2, int(self.tsek_mean - 3*self.tsek_std))
                        max_width = min(8, int(self.tsek_mean + 3*self.tsek_std))
                        min_area = max(6, int((self.tsek_mean - 2*self.tsek_std)**2))
                        max_area = min(30, int((self.tsek_mean + 2.5*self.tsek_std)**2))
                        ...
                        if (cbox[2] >= min_width and cbox[3] >= min_width and 
                            ...
                            print(f"[PUNCT DEBUG] Detected TSHEG at i={i}, box={cbox} [ADAPTIVE: w={min_width}-{max_width}, area={min_area}-{max_area}]")
                        else:
                            continue
                    else:
...
#         return cv.findContours(self.img_arr.copy(), mode=self._contour_mode , method=cv.CHAIN_APPROX_SIMPLE)
        # Convert image to proper format for OpenCV contours (CV_8UC1)
        img_copy = self.img_arr.copy()
        if img_copy.dtype != np.uint8:
            img_copy = img_copy.astype(np.uint8)
        
        # Convert from [0,1] range back to [0,255] for proper thresholding
        # The image was normalized to [0,1] range in namsel.py by dividing by 255
        if img_copy.max() <= 1.0:
            img_255 = (img_copy * 255).astype(np.uint8)
        else:
            img_255 = img_copy
            
        # Enhanced thresholding to better detect small characters like tshegs
        # Use adaptive thresholding which is more sensitive to local variations
        if img_255.mean() > 127:  # White background - invert for black text on white
            # Try adaptive thresholding first for better small character detection
            try:
                # Use adaptive Gaussian thresholding - better for varying illumination
                # More aggressive tuning: blockSize (11->7) and C (2->0) for maximum gap sensitivity
                img_binary = cv.adaptiveThreshold(img_255, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, 
                                                 cv.THRESH_BINARY_INV, 11, 2)
            except:
                try:
                    # Fallback to OTSU with manual adjustment for small characters
                    otsu_thresh, img_binary = cv.threshold(img_255, 0, 255, cv.THRESH_BINARY_INV + cv.THRESH_OTSU)
                    # If OTSU threshold is too high, reduce it to catch small/faint characters
                    if otsu_thresh > 150:
                        _, img_binary = cv.threshold(img_255, otsu_thresh - 20, 255, cv.THRESH_BINARY_INV)
                except:
                    # Final fallback to gentler fixed threshold
                    _, img_binary = cv.threshold(img_255, 120, 255, cv.THRESH_BINARY_INV)
        else:  # Already inverted or dark background
            try:
                # Use adaptive thresholding for dark backgrounds too
                # Reduced blockSize (11->9) and C (2->1) for more sensitive gap detection
                img_binary = cv.adaptiveThreshold(img_255, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, 
                                                 cv.THRESH_BINARY, 9, 1)
            except:
                try:
                    otsu_thresh, img_binary = cv.threshold(img_255, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)
                    # Adjust threshold for better small character detection
                    if otsu_thresh < 100:
                        _, img_binary = cv.threshold(img_255, otsu_thresh + 20, 255, cv.THRESH_BINARY)
                except:
                    _, img_binary = cv.threshold(img_255, 120, 255, cv.THRESH_BINARY)
        
        # Find contours on the properly thresholded binary image
        result = cv.findContours(img_binary, mode=self._contour_mode,
                               method=cv.CHAIN_APPROX_SIMPLE)
        ...
        # Focus on fixing existing tsheg detection instead of synthetic insertion
        # synthetic_contours = self._insert_gap_based_tsheg_contours(contours, img_binary)
        # 
        # # Add synthetic contours and update hierarchy
        # if synthetic_contours:
        #     # Convert contours to list if it's a tuple
        #     contours = list(contours)
        #     
        #     for synthetic_contour in synthetic_contours:
        #         contours.append(synthetic_contour)
        #         # Add hierarchy entry for synthetic contours
        #         if hierarchy is not None:
        #             new_hierarchy_entry = [-1, -1, -1, -1]  # No connections for synthetic contours
        #             hierarchy = np.append(hierarchy, [[new_hierarchy_entry]], axis=1)
