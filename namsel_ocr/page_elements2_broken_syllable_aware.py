...
class TshegPreprocessor:
    ...
    def preprocess(self, img_arr):
        ...
        # Gentle erosion only if we detect merged components (area > 250)
        ...
        large_merged = [box for box in original_boxes if box[2] * box[3] > 250]  # Likely merged syllable+tsheg
...
#                 cv.rectangle(self.img_arr, (x,y), (x+w, y+h), 0)
            ...
            # Check if this could be a valid Tibetan punctuation before filtering by size
                ...
                if quick_prd == '་':
                    ...
                    # Use statistical measurements if available
                    if hasattr(self, 'tsek_mean') and self.tsek_mean > 0:
                        # Generous thresholds for real tshegs (position already filtered)
                        min_width = 3  # Fixed minimum 3 pixels
                        max_width = 6  # Fixed maximum 6 pixels  
                        min_area = 9   # Fixed minimum area (covers both area=9 and area=12)
                        max_area = 25  # Fixed maximum area
                        
                        print(f"[TSHEG DEBUG] Checking tsheg i={i}, box={cbox}, area={area} against bounds w={min_width}-{max_width}, area={min_area}-{max_area}")
                        ...
                        if (cbox[2] >= min_width and cbox[3] >= min_width and 
                            ...
                            print(f"[PUNCT DEBUG] VALID TSHEG at i={i}, box={cbox} [PASSED: size+position OK]")
                        else:
                            print(f"[PUNCT DEBUG] FILTERED tsheg at i={i}, box={cbox} [FAILED: w={cbox[2]}, h={cbox[3]}, area={area}]")
                            continue
                    else:
                        # Fallback to fixed thresholds if no statistics available
                        if (cbox[2] >= 3 and cbox[3] >= 3 and 
                            cbox[2] <= 6 and cbox[3] <= 6 and 
                            area >= 9 and area <= 25):
                            self.small_contour_indices.append(i)
                            print(f"[PUNCT DEBUG] Detected TSHEG at i={i}, box={cbox} [FIXED THRESHOLD]")
                        else:
                            continue
                elif quick_prd == '།':
                    ...
                    # Add shad to small_contour_indices to avoid combination
                    self.small_contour_indices.append(i)
                else:
...
#                 quick_prd not in word_parts_set) and not self.low_ink :
                ...
                # All punctuation (including tsheg/shad) goes to small_contour_indices to avoid combination
                self.small_contour_indices.append(i)
...
#                    break
    ...
    def _contours(self):
        ...
        # Apply standard adaptive thresholding
        bin_img = cv.adaptiveThreshold(img_copy, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C,
                                      cv.THRESH_BINARY_INV, 11, 2)
        
        # Get contours using standard approach
        ...
        # try:
        #     merged_contours, index_mapping = self._merge_syllable_contours_with_mapping(contours)
        #     print(f"[SYLLABLE DEBUG] Merged {len(contours)} → {len(merged_contours)} contours")
        #     
        #     # Update contours and store index mapping for later use
        #     contours = merged_contours
        #     self.contour_index_mapping = index_mapping
        #     print(f"[SYLLABLE DEBUG] Index mapping created: {len(index_mapping)} entries")
...
#               self.small_coef * np.floor(self.tsek_std))) ] # ... and too small
    ...
    def _syllable_aware_processing(self, binary, contours, boxes):
        """
        Apply syllable-aware morphological operations to keep complex syllables intact.
        Instead of erosion (which splits), use dilation and closing to connect components.
        """
        # Identify potential syllable groups that should be connected
        syllable_groups = self._identify_syllable_groups(boxes)
        
        if not syllable_groups:
            print("[SYLLABLE DEBUG] No syllable groups found, using original binary")
            return binary
        
        # Create a copy for processing
        processed = binary.copy()
        
        # For each syllable group, apply connecting operations
        for group in syllable_groups:
            main_box, subscript_boxes = group
            print(f"[SYLLABLE DEBUG] Connecting syllable: main={main_box}, subscripts={len(subscript_boxes)}")
            
            # Create a mask for this syllable group
            mask = self._create_syllable_mask(processed.shape, main_box, subscript_boxes)
            
            # Apply morphological closing to connect components within the mask
            kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE, (3, 3))
            syllable_region = cv.bitwise_and(processed, mask)
            connected_syllable = cv.morphologyEx(syllable_region, cv.MORPH_CLOSE, kernel, iterations=1)
            
            # Replace the region in the processed image
            mask_inv = cv.bitwise_not(mask)
            processed = cv.bitwise_and(processed, mask_inv)  # Clear the region
            processed = cv.bitwise_or(processed, connected_syllable)  # Add connected syllable
        
        return processed
    
    def _identify_syllable_groups(self, boxes):
        """
        Identify groups of contours that should be connected as syllables.
        Returns list of (main_box, [subscript_boxes]) tuples.
        """
        syllable_groups = []
        used_indices = set()
        
        for i, main_box in enumerate(boxes):
            ...
            # Look for potential subscripts
            subscript_boxes = []
            subscript_indices = []
            
            for j, potential_sub in enumerate(boxes):
                if i == j or j in used_indices:
                    continue
                    
                sub_x, sub_y, sub_w, sub_h = potential_sub
                sub_area = sub_w * sub_h
                
                # Check if this could be a subscript
                vertical_distance = sub_y - (main_y + main_h)
                horizontal_overlap = self._calculate_horizontal_overlap(
                    (main_x, main_x + main_w), (sub_x, sub_x + sub_w)
                )
                
                # Conservative criteria for syllable grouping
                is_subscript = (
                    -10 <= vertical_distance <= 15 and  # Subscript below or slightly overlapping
                    horizontal_overlap > 0.3 and        # Good horizontal overlap
                    sub_area < 0.5 * main_area and      # Subscript smaller than main
                    sub_h < main_h and                  # Subscript not taller than main
                    main_area > 80                      # Main character large enough to have subscripts
                )
                
                if is_subscript:
                    subscript_boxes.append(potential_sub)
                    subscript_indices.append(j)
                    print(f"[SYLLABLE DEBUG] Found subscript for connection: main=({main_x},{main_y},{main_w},{main_h}) sub=({sub_x},{sub_y},{sub_w},{sub_h}) overlap={horizontal_overlap:.2f}")
            
            if subscript_boxes:
                syllable_groups.append((main_box, subscript_boxes))
                used_indices.add(i)
                used_indices.update(subscript_indices)
        
        return syllable_groups
    
    def _create_syllable_mask(self, image_shape, main_box, subscript_boxes):
        """Create a mask covering the syllable region."""
        mask = np.zeros(image_shape, dtype=np.uint8)
        
        # Combine all boxes to get the syllable bounding area
        all_boxes = [main_box] + subscript_boxes
        ...
        # Add some padding to ensure connection
        padding = 2
